...
export const editCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const course = await CoursModel.findByIdAndUpdate(
        courseId,
        {
          $set: data,
        },
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isCacheExist) {
        ...
        const course = await CoursModel.findById(req.params.id).select(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isCacheExist) {
        ...
        const courses = await CoursModel.find().select(
          "-courseData.videoUrl -courseData.suggestion -courseData.questions -courseData.links"
        );
...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const course = await CoursModel.findById(courseId);
...
export const addReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // check if the courseId is exist in user course list based on _id
